...
DEFAULT_MODEL_PATH = REPO_ROOT / "models" / "death_risk_5s_lgbm.txt"
DEFAULT_METRICS_PATH = REPO_ROOT / "data" / "ml" / "death_risk_5s_metrics.json"
DEFAULT_FEATURE_IMPORTANCE_PATH = (
    REPO_ROOT / "data" / "ml" / "death_risk_5s_feature_importance.json"
)
...
SIDE_FEATURE_CANDIDATES = ["player_side", "side"]
OPTIONAL_FEATURE_COLUMNS = ["prior_round_phase"]
BASE_CATEGORICAL_COLUMNS = ["map_name", "source_situation_type", "event_weapon"]
OPTIONAL_CATEGORICAL_COLUMNS = ["prior_round_phase"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a LightGBM binary classifier for death risk within 5 seconds.",
    )
    parser.add_argument(
        "--dataset",
        type=Path,
...
def select_feature_columns(dataset: pl.DataFrame) -> tuple[list[str], list[str]]:
    side_column = next(
        (column for column in SIDE_FEATURE_CANDIDATES if column in dataset.columns),
        None,
    )
    if side_column is None:
        raise SystemExit("Dataset must contain either player_side or side.")

    feature_columns = list(BASE_FEATURE_COLUMNS)
    feature_columns.insert(4, side_column)
    ...
    categorical_columns.insert(2, side_column)
    categorical_columns.extend(
        column for column in OPTIONAL_CATEGORICAL_COLUMNS if column in feature_columns
    )
...
def normalize_dataset(
    ...
    selected_columns = [MATCH_ID_COLUMN, TARGET_COLUMN, *feature_columns]
...
def assert_binary_target_for_training(y_train: np.ndarray) -> None:
    if len(np.unique(y_train)) != 2:
        raise SystemExit("Training split target must contain both positive and negative examples.")
...
def main() -> None:
    ...
    assert_binary_target_for_training(y_train)
